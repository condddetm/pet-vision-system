"""
Oxford-IIIT Pet 分类训练（自定义 SimpleCNN）
导出：backend/weights/cls_simple_cnn.pth
...
def parse_args():
    ...
    p.add_argument("--out", type=str, default="backend/weights/cls_simple_cnn.pth")
...
def main():
    ...
    print(f"Classification model: SimpleCNN (4-layer custom CNN, 随机初始化)")
    ...
    curves_path = Path(args.curves_out) if args.curves_out else Path("backend/artifacts/classification/curves_simple_cnn.json")
